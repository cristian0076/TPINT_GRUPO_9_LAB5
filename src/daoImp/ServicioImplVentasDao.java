...
public class ServicioImplVentasDao implements DaoVenta {
	...
	public List<Venta> obtenerTodasLasVentasSegunPagina(String pagina, String modoFiltro, String textoFiltro) {
		...
		case "2":
			listaVentas = (List<Venta>) session
					.createQuery(
							"SELECT c FROM Venta c INNER JOIN c.Id_Cliente a WHERE c.EstadoV = 1 AND a.Nombre_C LIKE '%"
									+ textoFiltro + "%'")
					.setFirstResult(Integer.parseInt(pagina) * 5).setMaxResults(5).list();
			break;
		case "3":
			listaVentas = (List<Venta>) session
					.createQuery(
							"SELECT c FROM Venta c INNER JOIN c.Id_usuario u WHERE c.EstadoV = 1 AND u.usuarioU LIKE '%"
									+ textoFiltro + "%'")
					.setFirstResult(Integer.parseInt(pagina) * 5).setMaxResults(5).list();
			break;
		case "4":
			listaVentas = (List<Venta>) session
					.createQuery("SELECT c FROM Venta c WHERE c.EstadoV = 1 AND c.Total_V LIKE '%" + textoFiltro + "%'")
					.setFirstResult(Integer.parseInt(pagina) * 5).setMaxResults(5).list();
			break;
		default:
	...
	public List<Venta> obtenerTodasLasVentas(String modoFiltro, String textoFiltro) {
		...
		case "1":
			...
			break;
		case "2":
			listaVentas = (List<Venta>) session.createQuery(
					"SELECT c FROM Venta c INNER JOIN c.Id_Cliente a WHERE c.EstadoV = 1 AND a.Nombre_C LIKE '%"
							+ textoFiltro + "%'")
					.list();
			break;
		...
		case "4":
			...
			break;
		default:
			listaVentas = (List<Venta>) session.createQuery("SELECT c FROM Venta c WHERE c.EstadoV = 1").list();
			break;
		}
	...
	public boolean actualizarVenta(int id) {
		...
		try {
			...
			List<VentaArticulo> listaVentas = (List<VentaArticulo>) session
					.createQuery("SELECT va FROM VentaArticulo va where va.ventaVA = " + id).list();
			...
			for (VentaArticulo ventaArticulo : listaVentas) {
				...
				List<Stock> listaStock = (List<Stock>) session
						.createQuery(
								"SELECT s FROM Stock s where s.articulo = " + ventaArticulo.getArticuloVA().getId())
						.list();
			...
			if (sinProblemas) {
				...
				for (VentaArticulo ventaArticulo : listaVentas) {
					...
					Query query = session
							.createSQLQuery("CALL DescontarStockVenta(:pCantidadVA, :pArticuloVA, :ventaarticuloid)");
					...
					query.setParameter("ventaarticuloid", ventaArticulo.getId());
	...
	public List<Venta> obtenerTodasLasVentasSegunPaginaYFecha(String pagina, String txtFiltroVentaContadorInicio,
		...
		try {
			...
			listaVentas = (List<Venta>) session
					.createQuery("SELECT c FROM Venta c WHERE c.EstadoV = 1 AND c.Fecha_V BETWEEN '" + txtFiltroVentaContadorInicio
							+ "' AND '" + txtFiltroVentaContadorFin+"'")
					.setFirstResult(Integer.parseInt(pagina) * 5).setMaxResults(5).list();
			ch.cerrarSession();
			return listaVentas;
		} catch (Exception e) {
		...
		try {
			...
			listaVentas = (List<Venta>) session
					.createQuery("SELECT c FROM Venta c WHERE c.EstadoV = 1 AND c.Fecha_V BETWEEN '"
							+ txtFiltroVentaContadorInicio + "' AND '" + txtFiltroVentaContadorFin+"'" )
					.list();
			ch.cerrarSession();
			return listaVentas;
		} catch (Exception e) {
