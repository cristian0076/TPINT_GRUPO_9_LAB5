...
<body style="height: 100vh; width: 100%;">
	<f:view
						...
						<form action="AgregarStock_ABMLStock.html" method="post">
							...
							<select class="form-control" name="txtProducto" required="required">
  									...
  									<c:forEach var="prod" items="${Productos}">
    									<option value="${prod.id}">${prod.nombreA} - ${prod.marcaA.nombreM}</option>
  									</c:forEach>
								</select>
								<span>Cantidad</span> 
								<input type="number" class="form-control"
									name="txtCantidad" placeholder="Cantidad"
									required="required"> 
									<span>Precio de compra</span> 
									<input
									type="number" class="form-control" name="txtPrecioC"
									placeholder="Precio" required="required">
								<span>Fecha de vencimiento</span> 
								<input type="date" class="form-control" name="txtVencimiento" id= "txtVencimiento" placeholder="Vencimiento" required="required" oninput="validateDate(this)">
							</div>
		...
		<!-- Boton agregar -->
		<button type="submit"  data-toggle="modal" data-target="#modalStock" class="btn btn-primary my-3 w-[30px] mx-auto"
			...
			<div class="input-group w-50 ml-3 mx-auto">
				<form action="Redireccionar_ABMLStock.html" class="input-group">
					<select class="custom-select ddlFiltroStock"
						id="ddlFiltroStock" name="ddlFiltroStock">
						...
						id="txtFiltroStock" name="txtFiltroStock">
		...
		<form action="Redireccionar_ABMLStock.html" method="post">
			<select class="custom-select ddlFiltroStock"
				id="ddlFiltroStock" name="ddlFiltroStock" hidden>
				<option selected value="0">Filtros</option>
				<option value="1">ID</option>
				<option value="2">Fecha vencimiento</option>
				<option value="3">Cantidad</option>
